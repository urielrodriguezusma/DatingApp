...
namespace Datting.api.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientsId)
        {
            var messages = await _context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos)
                                          .Include(u => u.Recipient).ThenInclude(p => p.Photos)
                                          .Where(m => m.RecipientId == userId && m.SenderId == recipientsId && m.RecipientDeleted == false ||
                                                 m.RecipientId == recipientsId && m.SenderId == userId && m.RecipientDeleted == false)
